...
export const StatisticsPage: React.FC = () => {
  ...
  const { data: items } = useSWR(
    ...
    async (path) =>
      (await get<{ groups: Groups; total: number }>(path)).data.groups.map(
        ({ happened_at, amount }) => ({
          x: happened_at,
  ...
  const { data: items2 } = useSWR(
    ...
    async (path) =>
      (await get<{ groups: Groups2; total: number }>(path)).data.groups.map(
        ({ tag_id, tag, amount }) => ({
          name: tag.name,
      ...
      <TimeRangePicker
        ...
        timeRanges={[
          ...
          },
          {
            ...
            key: {
              name: 'twoMonthsAgo',
              start: time().add(-2, 'month').firstDayOfMonth,
              end: time().add(-2, 'month').lastDayOfMonth.add(1, 'day')
            }
          },
          {
            text: '三个月前',
            key: {
              name: 'threeMonthsAgo',
              start: time().add(-3, 'month').firstDayOfMonth,
              end: time().add(-3, 'month').lastDayOfMonth.add(1, 'day')
            }
          }
        ]}
      />
